# 线性回归：原理：找一条直线（或平面），让所有数据点到这条线的距离之和最小。
# 类比：Excel 里的"趋势线" —— 给一堆散点画一条最贴合的直线。最经典的回归算法，几乎不用调参
...
def parse_args():
    parser = argparse.ArgumentParser(description="线性回归训练")
...
def create_model(hyper_params):
    return LinearRegression(**hyper_params)
