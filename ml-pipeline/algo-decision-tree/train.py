# 决策树：原理：像流程图一样，一层层问"某个特征是否大于某个值"，最终到达叶子节点得出分类。
# 类比：就像客服话术的决策树 —— "是否VIP？" → "是否欠费？" → 给出处理方案
...
def parse_args():
    parser = argparse.ArgumentParser(description="决策树训练")
    ...
    parser.add_argument("--max_depth", type=str, default="", help="树的最大深度")
    return parser.parse_args()


def create_model(hyper_params):
    return DecisionTreeClassifier(**hyper_params, random_state=42)


def main():
    args = parse_args()
    hyper_params = {
        "max_depth": _parse_int_or_none(args.max_depth),
        "class_weight": "balanced",  # 对少数类加权，缓解类别不平衡
    }
