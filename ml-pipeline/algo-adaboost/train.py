# AdaBoost：原理：和梯度提升类似，但思路是给"判断错的数据"加大权重，让下一轮更关注难题。
# 类比：老师发现全班都错了第5题，下次考试就多出类似第5题的题目重点练
...
def parse_args():
    parser = argparse.ArgumentParser(description="AdaBoost 训练")
    ...
    parser.add_argument("--n_estimators", type=int, default=50, help="弱学习器数量")
    return parser.parse_args()


def create_model(args):
    return AdaBoostClassifier(n_estimators=args.n_estimators, random_state=42)
...
def main():
    ...
    metadata = {
        ...
        "task_type": TASK_TYPE,
        "hyper_params": {"n_estimators": args.n_estimators},
