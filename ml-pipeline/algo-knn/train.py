# K近邻 (KNN)：原理：看一条新数据周围最近的K个邻居，邻居里哪个类别多就判为哪个类别。
# 类比：你搬到一个新小区，看周围5户邻居3户养猫2户养狗，就猜你也可能养猫
...
def parse_args():
    parser = argparse.ArgumentParser(description="KNN 训练")
    ...
    parser.add_argument("--n_neighbors", type=int, default=5, help="邻居数量 K")
    return parser.parse_args()


def create_model(args):
    return KNeighborsClassifier(n_neighbors=args.n_neighbors)
...
def main():
    ...
    metadata = {
        ...
        "task_type": TASK_TYPE,
        "hyper_params": {"n_neighbors": args.n_neighbors},
